...
void BlobTracking::process(const cv::Mat &img_input, const cv::Mat &img_mask, cv::Mat &img_output)
{
  ...
  cv::Mat morph;
  //IplConvKernel* morphKernel = cvCreateStructuringElementEx(5, 5, 1, 1, CV_SHAPE_ELLIPSE, NULL);
  //cvMorphologyEx(segmentated, segmentated, NULL, morphKernel, CV_MOP_OPEN, 1);
  cv::Mat element = cv::getStructuringElement( 2, cv::Size( 2*2 + 1, 2*2+1 ), cv::Point( 2,2 ));
  cv::morphologyEx(img_mask, morph, 2, element, cv::Point(-1,-1), 3);
  IplImage* segmentated = new IplImage(morph);
  //if(showBlobMask)
  ...
  if(showOutput)
   ...
   //cv::Mat imgResult(frame);
  cv::Mat imgResult;
  imgResult=cv::cvarrToMat(frame);
  imgResult.copyTo(img_output);
...
void BlobTracking::loadConfig()
{
  ...
  minArea = cvReadIntByName(fs, 0, "minArea", 1000);
  maxArea = cvReadIntByName(fs, 0, "maxArea", 5000);
